...
def profile(request):
    if request.user.is_authenticated:
        user = request.user
        orders = OrderDetails.objects.filter(userid=user.id).order_by('-date')
        for order in orders:
            order.total_amount = sum(item.amount for item in OrderItemMapping.objects.filter(orderDetails=order))

        return render(request, 'logauth/profile.html', {'user': user, 'orders': orders})
    else:
        return redirect('index')
